...
def draw_donut(R1, R2, init_r):
    dic = {}
    theta_arr = np.linspace(0, 2*pi, 60).tolist()
    circle = []
    surface_normal = []
    i = 0
...
def draw_box(w, l, h, init_r):
    width = np.linspace(-w/2, w/2, 15)
    length = np.linspace(-l/2, l/2, 15)
    height = np.linspace(-h/2, h/2, 15)
    dic = {}
    i = 0
    for x in width:
        for y in length:
